...
const SHEET_NAMES = ['Вручну додані', 'Товари Kiborg', 'Товари Viktailor', 'Товари Ncase'];
...
async function generateXML() {
  ...
  for (const name of SHEET_NAMES) {
    ...
    let filteredItems = FILTER_ENABLED
      ? items.filter(row =>
          row[COL_STOCK_QTY] === true ||
          row[COL_STOCK_QTY] === 'TRUE' ||
          row[COL_STOCK_QTY] === 'true'
        )
      : items;
  ...
  // Currencies
  shop.ele('currencies').ele('currency', { id: 'UAH', rate: '1' });
...
function sanitizeText(text) {
  ...
  return text
    ...
    .replace(/&nbsp;/gi, ' ')
    .replace(/&trade;/gi, '™')
    .replace(/&/g, '&amp;'); // щоб сирі & не ламали XML
}
